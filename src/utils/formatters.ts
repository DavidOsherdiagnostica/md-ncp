...
export function formatDetailedDrugInfo(drugDetails: GetSpecificDrugResponse): ProcessedDrug {
  return {
    ...
    activeIngredients: drugDetails.activeMetirals.map(ingredient => 
      `${ingredient.ingredientsDesc} ${ingredient.dosage}`
    ),
 ...
 */
export function formatSymptomHierarchy(categories: SymptomCategory[], popularSymptoms?: PopularSymptom[]) {
...
function generateSearchSuggestions(drugs: ProcessedDrug[], query: string): string[] {
  ...
  const hasGenericPotential = drugs.some(drug => 
    drug.activeIngredients.some(ingredient => 
      ingredient.includes('PARACETAMOL') || ingredient.includes('IBUPROFEN')
    )
  );
...
function generateResponseWarnings(data: unknown): string[] {
  return [
    SAFETY_CONFIG.DISCLAIMERS.GENERAL
  ];
}
 ...
 */
function generateNextActions(data: unknown): Array<{ tool: string; reason: string; parameters_hint: string }> {
