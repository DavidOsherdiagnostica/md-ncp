...
export const DrugRegistrationNumberSchema = z
  .string()
...
export function validateDrugName(input: string): string {
  try {
    ...
    throw new IsraelDrugsError(ErrorType.INVALID_INPUT, `Invalid drug name: ${input}`, {
 ...
 */
export function validateSymptomInput(
  category: string,
  symptom: string,
): { category: string; symptom: string } {
  try {
...
export function validatePageIndex(input: number): number {
  try {
    ...
    throw new IsraelDrugsError(ErrorType.INVALID_INPUT, `Invalid page index: ${input}`, {
      severity: ErrorSeverity.LOW,
      suggestions: ['Page numbering starts from 1, not 0', 'Use reasonable page numbers (< 1000)'],
...
export function validateAdministrationRoute(routeId: number): number {
  const validRoutes: number[] = Object.values(CLINICAL_MAPPINGS.ADMIN_ROUTES) as number[];
 ...
 */
export function validatePrescriptionPreference(
  preference: 'has_prescription' | 'otc_only' | 'either',
): boolean {
  switch (preference) {
    case 'otc_only':
...
export function validateSearchCriteria(criteria: {
  ...
  if (criteria.administrationRoute) {
    validatedCriteria.administrationRoute = validateAdministrationRoute(
      criteria.administrationRoute,
    );
  }
  ...
  if (!criteria.query && !criteria.atcCode && !criteria.administrationRoute) {
    ...
    suggestions.push(
      'Consider providing drug name, ATC code, or administration route for better results',
    );
  }
