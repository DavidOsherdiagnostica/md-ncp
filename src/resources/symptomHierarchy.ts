    ...
    async () => {
      try {
        ...
        return createComprehensiveErrorResponse(classifiedError, undefined, {
          resourceName: "symptom_hierarchy_guide"
        });
      }
    }
  );
...
async function generateSymptomHierarchyResource(): Promise<string> {
  ...
  try {
    ...
    const resource = {
      metadata: {
        ...
        total_symptoms: symptomData.reduce((sum: number, category: any) => sum + (category.list?.length || 0), 0),
        clinical_framework: "evidence_based_symptom_classification"
      },
      
      clinical_overview: {
        purpose: "Systematic symptom classification for clinical decision support and treatment pathway selection",
        scope: "Complete Israeli healthcare symptom taxonomy with treatment mappings",
        target_users: ["Primary care physicians", "Emergency medicine providers", "Specialists", "Clinical pharmacists", "Nursing staff"],
        clinical_importance: "Symptom-based approach enables systematic diagnosis and evidence-based treatment selection"
      },
    ...
    return JSON.stringify(await generateFallbackSymptomResource(), null, 2);
  }
}
  ...
  return {
    organizational_framework: {
      ...
      level_2_symptoms: {
        ...
        total_count: symptomData.reduce((sum: number, cat: any) => sum + (cat.list?.length || 0), 0),
  ...
  return {
    anatomical_systems: {
      ...
      musculoskeletal: {
        ...
        treatment_complexity: "Often multimodal approach with pharmacological and non-pharmacological interventions"
      },
  ...
  return {
    pathway_framework: {
      assessment_phase: {
        ...
        risk_stratification: "Identification of high-risk presentations requiring immediate attention",
...
function generateQualityIndicators(): any {
  return {
    clinical_effectiveness: {
      ...
      functional_improvement: "Improvement in activities of daily living and quality of life measures",
      time_to_improvement: "Duration from treatment initiation to clinically meaningful improvement",
    ...
    efficiency_measures: {
      diagnostic_accuracy: "Proportion of correct initial diagnoses and appropriate treatment selection",
    ...
    population_health: {
      ...
      chronic_disease_management: "Long-term outcomes for patients with chronic symptomatic conditions"
    }
  };
}
...
function filterCategoriesBySystem(symptomData: any[], keywords: string[]): any[] {
  return symptomData.filter((category: any) => 
    keywords.some(keyword => 
      category.bySymptomMain.includes(keyword)
    )
  );
}
...
function generateCategoryDistribution(symptomData: any[]): any {
  const distribution = symptomData.map((category: any) => ({
    ...
    symptom_count: category.list?.length || 0,
    relative_size: ((category.list?.length || 0) / symptomData.reduce((sum: number, cat: any) => sum + (cat.list?.length || 0), 0) * 100).toFixed(1) + '%'
  }));
...
function mapSeverityGradients(symptomData: any[]): any {
  return {
    mild_to_severe: "Most symptoms can range from mild self-limiting to severe requiring intervention",
...
function identifyMedicationClasses(categoryName: string): string[] {
  ...
  if (categoryName.includes("גרון")) return ["Local anesthetics", "Antiseptics", "Anti-inflammatories"];
...
function identifyHighDemandAreas(popularSymptoms: any[]): string[] {
  const topCategories = popularSymptoms
    .slice(0, 5)
    .map((symptom: any) => symptom.bySymptomMain);
