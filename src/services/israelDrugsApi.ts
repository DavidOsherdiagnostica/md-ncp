...
} from '../types/api.js'; // Reverted to api.js
...
export class IsraelDrugsApiClient {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (!responseText.trim()) {
        throw new IsraelDrugsError(ErrorType.API_INVALID_RESPONSE, 'API returned empty response', {
      ...
      try {
        ...
        throw new IsraelDrugsError(ErrorType.API_INVALID_RESPONSE, 'API returned invalid JSON', {
          severity: ErrorSeverity.HIGH,
          correlationId,
  ...
  async getImageUrl(imageName: string): Promise<string> {
    if (!imageName || imageName.trim() === '') {
      throw new IsraelDrugsError(ErrorType.INVALID_INPUT, 'Image name cannot be empty', {
  ...
  async searchByName(request: SearchByNameRequest): Promise<SearchByNameResponse> {
    ...
    return this.makeRequest<SearchByNameResponse>(API_CONFIG.ENDPOINTS.SEARCH_BY_NAME, request);
  }
  ...
  async searchGeneric(request: SearchGenericRequest): Promise<SearchGenericResponse> {
    ...
    return this.makeRequest<SearchGenericResponse>(API_CONFIG.ENDPOINTS.SEARCH_GENERIC, request);
  }
  ...
  async getBySymptom(request: GetBySymptomRequest): Promise<GetBySymptomResponse> {
    return this.makeRequest<GetBySymptomResponse>(API_CONFIG.ENDPOINTS.GET_BY_SYMPTOM, request);
  }
  ...
  async getAtcList(): Promise<GetAtcListResponse> {
    return this.makeRequest<GetAtcListResponse>(API_CONFIG.ENDPOINTS.GET_ATC_LIST, {});
  }
  ...
  async getPackageList(): Promise<GetPackageListResponse> {
    return this.makeRequest<GetPackageListResponse>(API_CONFIG.ENDPOINTS.GET_PACKAGE_LIST, {});
  }
  ...
  async getMatanList(): Promise<GetMatanListResponse> {
    return this.makeRequest<GetMatanListResponse>(API_CONFIG.ENDPOINTS.GET_MATAN_LIST, {});
  }
  ...
  private validateAutocompleteRequest(request: AutocompleteRequest): void {
    if (!request.val || request.val.trim().length === 0) {
      throw new IsraelDrugsError(ErrorType.INVALID_INPUT, 'Search value cannot be empty', {
        severity: ErrorSeverity.LOW,
    ...
    if (request.val.length > 100) {
      throw new IsraelDrugsError(ErrorType.INVALID_INPUT, 'Search value too long', {
        severity: ErrorSeverity.LOW,
  ...
  private validateSearchByNameRequest(request: SearchByNameRequest): void {
    if (!request.val || request.val.trim().length === 0) {
      throw new IsraelDrugsError(ErrorType.INVALID_INPUT, 'Drug name cannot be empty', {
  ...
  private validatePopularSymptomsRequest(request: GetFastSearchPopularSymptomsRequest): void {
    if (request.rowCount < 1 || request.rowCount > 100) {
      throw new IsraelDrugsError(ErrorType.INVALID_INPUT, 'Row count must be between 1 and 100', {
        severity: ErrorSeverity.LOW,
