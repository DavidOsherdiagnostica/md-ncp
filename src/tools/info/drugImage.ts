...
type ValidatedVerifyDrugVisualIdentityInput = VerifyDrugVisualIdentityInput & {
  image_purpose: NonNullable<VerifyDrugVisualIdentityInput['image_purpose']>;
};
...
**Clinical Context:** This tool should be used whenever visual confirmation of medication identity is required for safety, education, or verification purposes. Essential component of comprehensive medication management and error prevention strategies.`,
      inputSchema: VerifyDrugVisualIdentitySchema.shape,
    },
    async (input: VerifyDrugVisualIdentityInput) => {
      ...
      try {
        ...
        const validatedVisualIdentityInput: ValidatedVerifyDrugVisualIdentityInput = validatedInput as ValidatedVerifyDrugVisualIdentityInput;

        // Validate drug registration number format
        const validatedRegNumber = validateDrugRegistrationNumber(
          validatedVisualIdentityInput.drug_registration_number,
        );

        // Retrieve visual identification information
        const visualInfo = await retrieveVisualIdentificationInfo(
          validatedRegNumber,
          validatedVisualIdentityInput,
        );

        // Create comprehensive visual identification response
        return createVisualIdentificationResponse(visualInfo, validatedVisualIdentityInput, warnings, startTime);
      } catch (error) {
...
async function retrieveVisualIdentificationInfo(
  ...
  userInput: ValidatedVerifyDrugVisualIdentityInput,
...
async function attemptImageFallback(
  registrationNumber: string,
): Promise<VisualIdentificationInfo | null> {
  try {
    const apiClient = getApiClient();
...
function createFallbackVisualInfo(basicInfo: any): VisualIdentificationInfo {
  return {
    ...
    packaging_information: {
      package_types:
        basicInfo.packages?.map((pkg: string) => ({
          description: pkg,
          material: 'Unknown',
          quantity: pkg,
...
function createVisualIdentificationResponse(
  ...
  const response: McpResponse<VisualIdentificationInfo> = {
    success: true,
    data: visualInfo,
    content: [{ type: 'text', text: JSON.stringify(visualInfo, null, 2) }], // Added content field
    metadata: {
...
function assessOverallImageQuality(images: ProcessedImage[]): string {
  if (images.length === 0 || images[0] === undefined || images[0].url === null) {
  ...
  const averageQuality = qualityScores.reduce((a: number, b: number) => a + b, 0) / qualityScores.length;
...
function generateVisualIdentificationNotes(
  ...
  if (
    visualInfo.identification_images.length === 0 ||
    visualInfo.identification_images[0] === undefined ||
    visualInfo.identification_images[0].url === null
  ) {
...
function generateVisualIdentificationWarnings(
  ...
  if (visualInfo.identification_images.length === 0) {
    warnings.push(
      'No visual identification images available - use alternative verification methods',
    );
  }
