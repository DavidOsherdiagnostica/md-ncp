...
type ValidatedGetComprehensiveDrugInfoInput = GetComprehensiveDrugInfoInput & {
  info_depth: NonNullable<GetComprehensiveDrugInfoInput['info_depth']>;
  include_clinical_data: NonNullable<GetComprehensiveDrugInfoInput['include_clinical_data']>;
  language_preference: NonNullable<GetComprehensiveDrugInfoInput['language_preference']>;
};
...
**Clinical Context:** This tool should be used when comprehensive medication information is required for prescribing decisions, patient education, safety assessments, or regulatory compliance verification.`,
      inputSchema: GetComprehensiveDrugInfoSchema.shape,
    },
    async (input: GetComprehensiveDrugInfoInput) => {
      ...
      try {
        ...
        const validatedDrugInfoInput: ValidatedGetComprehensiveDrugInfoInput = validatedInput as ValidatedGetComprehensiveDrugInfoInput;

        // Validate drug registration number format
        const validatedRegNumber = validateDrugRegistrationNumber(
          validatedDrugInfoInput.drug_registration_number,
        );

        // Execute comprehensive drug information retrieval
        const drugDetails = await retrieveComprehensiveDrugInfo(validatedRegNumber, validatedDrugInfoInput);

        // Format response with clinical intelligence
        const formatter = getResponseFormatter();
        const formattedResponse = formatter.formatDrugDetailsResponse(drugDetails, startTime);

        // Enhance with comprehensive clinical analysis
        return enhanceComprehensiveResponse(formattedResponse, validatedDrugInfoInput, warnings);
      } catch (error) {
...
async function retrieveComprehensiveDrugInfo(
  ...
  userInput: ValidatedGetComprehensiveDrugInfoInput,
): Promise<any> {
...
function convertBasicToDetailed(basicInfo: any): any {
  ...
  return {
    ...
    activeMetirals:
      basicInfo.activeComponents?.map((comp: any) => ({
...
async function generateClinicalAnalysis(
  ...
  return {
    ...
    pharmacological_profile: {
      active_ingredients:
        drugDetails.activeMetirals?.map((ingredient: any) => ({
...
async function generateRegulatoryProfile(drugDetails: any): Promise<Record<string, unknown>> {
  return {
    ...
    manufacturer_information: {
      ...
      manufacturing_sites:
        drugDetails.manufacturers?.map((mfg: any) => ({
    ...
    documentation: {
      patient_leaflets:
        drugDetails.brochure
          ?.filter((doc: any) => doc.type === 'עלון לצרכן')
          .map((doc: any) => ({
            language: doc.lng,
            url: doc.url,
      ...
      professional_information:
        drugDetails.brochure
          ?.filter((doc: any) => doc.type === 'עלון לרופא')
          .map((doc: any) => ({
            language: doc.lng,
            url: doc.url,
            last_updated: new Date(doc.updateDate).toISOString(),
      ...
      safety_updates:
        drugDetails.brochure
          ?.filter((doc: any) => doc.type === 'החמרה לעלון')
          .map((doc: any) => ({
            language: doc.lng,
            url: doc.url,
  ...
  return {
    product_quality: {
      packaging_information:
        drugDetails.packages?.map((pkg: any) => ({
    ...
    visual_identification: {
      product_images:
        drugDetails.images?.map((img: any) => `${API_CONFIG.IMAGES_BASE_URL}/${img.url}`) || [],
        ...
        safety_assessment: {
          ...
          clinical_safety: {
            prescription_requirement:
              enrichedInfo.safety_assessment.clinical_safety.prescription_requirement,
...
function assessEmergencyUse(drugDetails: any): string {
  return drugDetails.isPrescription
    ? 'prescription_required_emergency'
    : 'otc_emergency_appropriate';
}
...
function generateClinicalDecisionSupport(
  ...
  const support: ClinicalDecisionSupport = {
