...
type ValidatedSuggestDrugNamesInput = SuggestDrugNamesInput & {
  search_type: NonNullable<SuggestDrugNamesInput['search_type']>;
  max_suggestions: NonNullable<SuggestDrugNamesInput['max_suggestions']>;
};
...
**Clinical Context:** Medication name accuracy is critical for patient safety. This tool should be used whenever there's uncertainty about exact drug spelling or when exploring therapeutic alternatives.`,
      inputSchema: SuggestDrugNamesSchema.shape,
    },
    async (input: SuggestDrugNamesInput) => {
      ...
      try {
        ...
        const validatedAutocompleteInput: ValidatedSuggestDrugNamesInput = validatedInput as ValidatedSuggestDrugNamesInput;
        const sanitizedQuery = sanitizeSearchQuery(validatedAutocompleteInput.partial_name);

        // Transform MCP request to API format
        const apiRequest = {
          val: sanitizedQuery,
          isSearchTradeName: validatedAutocompleteInput.search_type === 'active_ingredients' ? '0' : '1',
          isSearchTradeMarkiv: validatedAutocompleteInput.search_type === 'trade_names' ? '0' : '1',
        };

        // Call API
        const apiClient = getApiClient();
        const apiResponse = await apiClient.searchBoxAutocomplete(apiRequest as any);

        // Process and format response
        const suggestions = apiResponse.results
          .slice(0, validatedAutocompleteInput.max_suggestions)
          .map((suggestion, index) => ({
            name: suggestion,
            confidence: calculateConfidence(suggestion, sanitizedQuery),
            rank: index + 1,
            name_type: determineNameType(suggestion, validatedAutocompleteInput.search_type),
            similarity_score: calculateSimilarity(suggestion, sanitizedQuery),
          }));

        // Create comprehensive response
        const responseData = {
          query: sanitizedQuery,
          search_type: validatedAutocompleteInput.search_type,
          suggestions: suggestions,
          total_suggestions: suggestions.length,
          search_strategy: determineSearchStrategy(validatedAutocompleteInput),
          spelling_analysis: analyzeSpelling(sanitizedQuery, suggestions),
          clinical_considerations: generateClinicalConsiderations(suggestions),
        };

        const formatter = getResponseFormatter();
        return createAutocompleteResponse(responseData, startTime);
      } catch (error) {
...
class AutocompleteToolHelper {
  ...
  private levenshteinDistance(str1: string, str2: string): number {
    ...
    // Create a 2D array (matrix) and initialize all cells to 0
    ...
    for (let j = 1; j <= n; j++) {
      for (let i = 1; i <= m; i++) {
        ...
        matrix[j]![i] = Math.min(
          ...
          (matrix[j - 1]![i - 1] as number) + indicator // Substitution or Match
        );
      }
    }

    return matrix[n]![m] as number;
  }
  ...
  private determineNameType(
    suggestion: string,
    searchType: ValidatedSuggestDrugNamesInput['search_type'],
   ...
   */
  private determineSearchStrategy(input: ValidatedSuggestDrugNamesInput): string {
  ...
  private hasTransliterationIssues(query: string, suggestion: string): boolean {
    ...
    return transliterationPairs.some(
      ([a, b]) =>
        (query.includes(a as string) && suggestion.includes(b as string)) ||
        (query.includes(b as string) && suggestion.includes(a as string)),
    );
  }
