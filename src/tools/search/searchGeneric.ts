...
import { ExploreGenericAlternativesSchema, ExploreGenericAlternativesInput, McpResponse } from "../../types/mcp.js";
...
**Clinical Context:** This tool is essential for therapeutic optimization, formulary compliance, cost containment, and ensuring patient access to clinically appropriate alternatives when primary therapy is unavailable or unaffordable.`,
      inputSchema: ExploreGenericAlternativesSchema
    },
    async (input: ExploreGenericAlternativesInput) => {
      ...
      try {
        ...
        const processedCriteria = await processSearchCriteria(validatedInput.search_criteria);
        
        // Execute multi-strategy search for comprehensive alternatives
        const searchResults = await executeAlternativesSearch(processedCriteria, validatedInput);
        ...
        const formattedResponse = formatter.formatDrugSearchResponse(
          ...
          generateSearchQueryDescription(processedCriteria),
          {
            ...
            healthBasketOnly: validatedInput.comparison_criteria?.health_basket_priority || false,
            pageIndex: 1
          },
          startTime
        );
        ...
        return enhanceAlternativesResponse(formattedResponse, validatedInput, processedCriteria, warnings);
...
async function executeAlternativesSearch(
  ...
  userInput: ExploreGenericAlternativesInput
): Promise<any> {
...
async function resolveReferenceDrug(drugName: string): Promise<ProcessedSearchCriteria> {
  ...
  try {
    ...
    throw new Error(`Failed to resolve reference drug "${drugName}": ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function generateSubstitutionGuidance(
  ...
  drugs.forEach((drug: any) => {
    if (!drug.isActive) {
      ...
      guidance.consider_with_monitoring.push(`${drug.hebrewName} - requires prescription but covered`);
    } else {
      guidance.requires_clinical_evaluation.push(`${drug.hebrewName} - full cost consideration needed`);
    }
  });
...
function generateCostOptimization(
  ...
  const basketOptions = pricesWithValues.filter(d => d.inBasket);
  const fullCostOptions = pricesWithValues.filter(d => !d.inBasket);
  
  return {
    pricing_analysis: {
      cheapest_option: pricesWithValues.reduce((min, current) => 
        current.price < min.price ? current : min
      ),
      most_expensive: pricesWithValues.reduce((max, current) => 
        current.price > max.price ? current : max
      ),
      average_price: pricesWithValues.reduce((sum, d) => sum + d.price, 0) / pricesWithValues.length
    },
    cost_savings: {
      basket_vs_full_cost: basketOptions.length > 0 && fullCostOptions.length > 0 ? 
        calculatePotentialSavings(basketOptions, fullCostOptions) : null,
...
function countGenericOptions(drugs: any[]): number {
  ...
  return drugs.filter((d: any) => 
    d.englishName && d.englishName.toUpperCase() === d.englishName
  ).length;
}
...
function assessSafetyProfileSimilarity(drugs: any[]): string {
  const hasActiveWarnings = drugs.some((d: any) => 
    d.clinicalInfo.warnings && d.clinicalInfo.warnings.length > 0
  );
...
function calculatePotentialSavings(basketOptions: any[], fullCostOptions: any[]): Record<string, number> {
  const avgBasketPrice = basketOptions.reduce((sum, d) => sum + d.price, 0) / basketOptions.length;
  const avgFullPrice = fullCostOptions.reduce((sum, d) => sum + d.price, 0) / fullCostOptions.length;
...
interface ProcessedSearchCriteria {
  searchType: "active_ingredient" | "atc_code" | "administration_route" | "reference_drug" | "unknown";
  primaryValue: string;
  routeId?: number;
