...
**Clinical Context:** This tool should be used when healthcare providers or patients need comprehensive information about specific medications, including verification of prescriptions, checking alternatives, or understanding cost implications.`,
      inputSchema: DiscoverDrugByNameSchema
    },
    async (input: DiscoverDrugByNameInput) => {
      ...
      try {
        ...
        const sanitizedQuery = validateDrugName(validatedInput.medication_query);
        
        // Transform MCP preferences to API format
        const apiRequest = {
          val: sanitizedQuery,
          prescription: validatePrescriptionPreference(
            validatedInput.patient_preferences?.prescription_access || "either"
          ),
          healthServices: validateHealthBasketPreference(
            validatedInput.patient_preferences?.budget_conscious || false
          ),
          pageIndex: validatePageIndex(1), // Start with first page
          orderBy: determineOrderBy(validatedInput.search_scope)
        };
        
        // Execute search with intelligent retry logic
        const searchResults = await executeIntelligentSearch(apiRequest, validatedInput);
        ...
        const formattedResponse = formatter.formatDrugSearchResponse(
          ...
          sanitizedQuery,
          {
            prescriptionAccess: validatedInput.patient_preferences?.prescription_access,
            healthBasketOnly: validatedInput.patient_preferences?.budget_conscious,
            pageIndex: 1
          },
          startTime
        );
        ...
        return enhanceSearchResponse(formattedResponse, validatedInput, warnings);
...
async function executeIntelligentSearch(
  baseRequest: any,
  userInput: DiscoverDrugByNameInput
): Promise<any> {
  ...
  try {
    ...
    if (primaryResult.results && primaryResult.results.length > MCP_CONFIG.RESPONSE_LIMITS.MAX_SEARCH_RESULTS) {
...
function generateSearchIntelligence(
  ...
  return {
    result_analysis: {
      ...
      exact_matches: drugs.filter((d: any) => 
...
function generatePatientGuidance(
  ...
  guidance.safety_reminders.push("Always verify medication information with healthcare professionals");
...
function generateRefinementSuggestions(
  ...
  const hasPriceVariation = drugs.some((d: any) => d.maxPrice !== null) && 
                           drugs.some((d: any) => d.maxPrice === null);
