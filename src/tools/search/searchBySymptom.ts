...
import { FindDrugsForSymptomSchema, FindDrugsForSymptomInput, McpResponse } from "../../types/mcp.js";
...
**Clinical Context:** This tool should be used when seeking evidence-based treatment options for specific symptoms. Results include safety profiles, contraindications, and guidance for appropriate medical supervision.`,
      inputSchema: FindDrugsForSymptomSchema
    },
    async (input: FindDrugsForSymptomInput) => {
      ...
      try {
        ...
        const { category, symptom } = validateSymptomInput(
          validatedInput.primary_category,
          validatedInput.specific_symptom
        );
        ...
        const apiRequest = {
          ...
          healthServices: validateHealthBasketPreference(
            validatedInput.treatment_preferences?.health_basket_only || false
          ),
          pageIndex: validatePageIndex(1),
          prescription: determinePrescriBtionFilter(validatedInput.treatment_preferences?.otc_preferred),
          orderBy: 5 // Use popularity-based ordering for symptoms
        };
        
        // Execute symptom-based search with clinical intelligence
        const searchResults = await executeSymptomSearch(apiRequest, validatedInput);
        ...
        const formattedResponse = formatter.formatDrugSearchResponse(
          ...
          `${category} → ${symptom}`,
          {
            prescriptionAccess: validatedInput.treatment_preferences?.otc_preferred ? "otc_only" : "either",
            healthBasketOnly: validatedInput.treatment_preferences?.health_basket_only,
            pageIndex: 1
          },
          startTime
        );
        ...
        return enhanceSymptomResponse(formattedResponse, validatedInput, warnings);
...
async function executeSymptomSearch(
  baseRequest: any,
  userInput: FindDrugsForSymptomInput
): Promise<any> {
...
function generateTreatmentStrategy(
  ...
  strategy.self_care_measures = generateSelfCareMeasures(userInput.primary_category, userInput.specific_symptom);
...
function generateSymptomSpecificNotes(userInput: FindDrugsForSymptomInput): string[] {
  const notes: string[] = [];
