...
type ValidatedListAdministrationRoutesInput = ListAdministrationRoutesInput & {
  include_descriptions: NonNullable<ListAdministrationRoutesInput['include_descriptions']>;
  common_only: NonNullable<ListAdministrationRoutesInput['common_only']>;
};
...
**Clinical Context:** This tool supports optimal medication delivery planning, ensures appropriate route selection for patient-specific needs, and provides essential guidance for safe and effective medication administration across diverse clinical scenarios.`,
      inputSchema: ListAdministrationRoutesSchema.shape,
    },
    async (input: ListAdministrationRoutesInput) => {
      ...
      try {
        ...
        const validatedRoutesInput: ValidatedListAdministrationRoutesInput = validatedInput as ValidatedListAdministrationRoutesInput;

        // Execute administration routes discovery
        const routesData = await executeAdministrationRoutesDiscovery(validatedRoutesInput);
        ...
        return enhanceAdministrationRoutesResponse(formattedResponse, validatedRoutesInput, warnings);
      } catch (error) {
...
async function executeAdministrationRoutesDiscovery(
  userInput: ValidatedListAdministrationRoutesInput,
): Promise<any[]> {
...
async function processAdministrationRoutes(
  ...
  if (userInput.complexity_level && userInput.complexity_level !== 'all') {
  ...
  if (userInput.setting_requirements && userInput.setting_requirements !== 'all') {
    processedData = filterBySettingRequirements(processedData, userInput.setting_requirements);
  }

  // Apply onset requirements filtering if specified
  if (userInput.onset_requirements && userInput.onset_requirements !== 'any') {
    processedData = filterByOnsetRequirements(processedData, userInput.onset_requirements);
  }
...
function getManualDexterityRequirements(routeText: string): string {
  return ['עורי', 'עיני', 'אוזני'].some((route) => routeText.includes(route))
    ? 'basic_manual_dexterity'
    : 'professional_administration';
}

function getCognitiveRequirements(routeText: string): string {
  return routeText.includes('פומי')
    ? 'understanding_of_instructions'
    : 'minimal_cognitive_requirements';
}
...
function getCostConsiderations(routeText: string): string {
  return routeText.includes('תוך-ורידי')
    ? 'higher_administration_costs'
    : 'standard_administration_costs';
}
...
function getFirstPassEffect(routeText: string): string {
  return routeText.includes('פומי')
    ? 'significant_first_pass_metabolism'
    : 'minimal_first_pass_effect';
}
...
function getPediatricSafety(routeText: string): string {
  return routeText.includes('פומי')
    ? 'ensure_age_appropriate_formulation'
    : 'pediatric_specialist_consultation';
}
...
function getOverdoseConsiderations(routeText: string): string[] {
  if (routeText.includes('תוך-ורידי'))
    return ['Cannot retrieve medication', 'Supportive care priority'];
  if (routeText.includes('פומי'))
    return ['Consider gastric lavage if recent', 'Activated charcoal if appropriate'];
...
function getCaregiverTrainingNeeds(routeText: string): string[] {
  if (routeText.includes('פומי'))
    return ['Proper dosing', 'Timing of administration', 'Side effect recognition'];
  if (routeText.includes('עורי'))
    return ['Application technique', 'Skin preparation', 'Monitoring for reactions'];
...
function getFacilityCapabilities(routeText: string): string[] {
  if (routeText.includes('תוך-ורידי'))
    return ['IV equipment', 'Emergency supplies', 'Trained nursing staff'];
  return ['Basic medication storage', 'Standard medical supplies'];
}

function getEquipmentNeeds(routeText: string): string[] {
  if (routeText.includes('תוך-ורידי'))
    return ['IV catheters', 'Infusion pumps', 'Monitoring equipment'];
...
function getStaffTraining(routeText: string): string[] {
  return [
    'Route-specific technique training',
...
function assessRouteFilterEffectiveness(
  routes: any[],
  userInput: ValidatedListAdministrationRoutesInput,
): string {
  const hasFilters =
    userInput.complexity_level !== 'all' ||
    userInput.patient_age_group !== 'all' ||
    userInput.setting_requirements !== 'all' ||
    userInput.onset_requirements !== 'any';
...
function identifyFirstLineRoutes(
  routes: any[],
  userInput: ValidatedListAdministrationRoutesInput,
): string[] {
...
function identifyAlternativeRoutes(
  routes: any[],
  userInput: ValidatedListAdministrationRoutesInput,
): string[] {
  return routes
...
function identifyLastResortRoutes(
  routes: any[],
  userInput: ValidatedListAdministrationRoutesInput,
): string[] {
  return routes
...
function generateAdministrationRoutesNotes(userInput: ValidatedListAdministrationRoutesInput): string[] {
  const notes: string[] = [];
  ...
  if (userInput.onset_requirements && userInput.onset_requirements !== 'any') {
    notes.push(`Routes filtered by ${userInput.onset_requirements} onset requirements`);
  }
...
function enhanceAdministrationRoutesNextActions(
  ...
  if (userInput.onset_requirements && userInput.onset_requirements !== 'any') {
    enhancedActions.push({
      tool: 'discover_drug_by_name',
      reason: `Find drugs with ${userInput.onset_requirements} onset characteristics`,
      parameters_hint: 'Search for medications based on onset of action',
    });
  }
...
function filterByOnsetRequirements(
  routesData: any[],
  onsetRequirement: 'immediate' | 'rapid' | 'gradual' | 'sustained' | 'any',
): any[] {
  if (onsetRequirement === 'any') return routesData;

  return routesData.filter((route: any) => {
    const routeOnset = getOnsetCharacteristics(route.text);
    // Simplified logic: map routeOnset to the desired onsetRequirement
    if (onsetRequirement === 'immediate' && routeOnset.includes('immediate')) return true;
    if (onsetRequirement === 'rapid' && routeOnset.includes('rapid')) return true;
    if (onsetRequirement === 'gradual' && routeOnset.includes('gradual')) return true;
    if (onsetRequirement === 'sustained' && routeOnset.includes('duration')) return true;
    return false;
  });
}
